...
module.exports = function (source) {
    ...
    [
        ...source.matchAll(/roots\.register\.(plugin|block|format|filter|variation|style)s\(['"`](.*)['"`]\)/g),
    ].map(([match, type, query]) => {
        source = source.replace(match, makeCodeString(type, query));
    });
    [...source.matchAll(/roots\.register\.notify\(false\)/g)]?.map(([match]) => {
        if (!match)
            return;
...
const makeCodeString = (type, query, enabled = true) => `

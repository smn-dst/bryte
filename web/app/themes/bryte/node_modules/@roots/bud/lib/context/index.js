...
export default async function make(options = {}) {
    ...
    try {
        ...
        if (manifest?.bud?.paths?.basedir) {
            ...
            logger
                .scope(`bootstrap`)
                .log(`🏗️`, `rebuilding context`, `based on bud.paths.basedir sourced from`, join(targetPath, `package.json`));
            return await make({
                basedir: targetPath,
                ...(options ?? {}),
            });
        ...
        logger
            .scope(`bootstrap`)
            .warn(`📦`, `no package.json found at ${join(paths.basedir, `package.json`)}`);
    }
